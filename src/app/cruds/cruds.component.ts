...
export class CrudsComponent implements OnInit {
  ...
  studentList: Student[] = [];
  ngOnInit(): void {
    const localData = localStorage.getItem('crudsabin');
    if (localData != null) {
  ...
  onDelete(item: Student) {
    ...
    if (isDelet) {
      ...
      localStorage.setItem('crudsabin', JSON.stringify(this.studentList));
    }
  }
  ...
  saveStudent() {
    debugger;
    const isLocalPresent = localStorage.getItem('crudsabin');
    if (isLocalPresent != null) {
      const oldArray = JSON.parse(isLocalPresent);
      this.studentObj.id = oldArray.length + 1;
      oldArray.push(this.studentObj);
      this.studentList = oldArray;
      localStorage.setItem('crudsabin', JSON.stringify(oldArray));
    } else {
      const newArr = [];
      newArr.push(this.studentObj);
      this.studentObj.id = 1;
      this.studentList = newArr;
      localStorage.setItem('crudsabin', JSON.stringify(newArr));
    }
    this.closeModel();
  }
}
